...
mapXYCenMimic <- function(start, end, y, x, yfactor, r2, pts, cenFormat = "triangle") {
  ...
  for (counter in start:end) {
    ...
    if (cenFormat == "triangle") {
      ...
      roundedX[[counter]] <- c(xy_1[, 1], minXmod, xy_1b[, 1], (xy_2b[, 1]), xy_2[, 1],
        (xy_3[, 1]), (xy_3b[, 1]), minXmod + halfmaxXMod, xy_4b[, 1], xy_4[, 1]) # opposite of cen.
      roundedY[[counter]] <- c(xy_1[, 2], minYmod, xy_1b[, 2], (xy_2b[, 2]), xy_2[, 2],
        (xy_3[, 2]), (xy_3b[, 2]), minYmod + halfmaxYMod, xy_4b[, 2], xy_4[, 2])
    }
