...
makedfMarkColor <- function(dfMarkColor, markNames, colorstoremove = NA,
  ...
  manualcolors <- c(
    "black", "forestgreen", "orange", "cornflowerblue",
  ...
  if (length(areNA)) {
    dfMarkColorNew[areNA, ]$markColor <- dfMarkColor$markColor[match(toupper(dfMarkColorNew[areNA, ]$markNameNP),
      toupper(dfMarkColor$markName))]
  }

  if (!missing(reserveDF)) {
    dfMarkColorNew$markColor2 <- reserveDF$markColor[match(
      toupper(dfMarkColorNew$markName),
